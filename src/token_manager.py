...
class TokenManager:
    ...
    def get_user_usage(self, user_id: int, model: str, days: Optional[int] = None) -> Dict[str, int]:
        ...
        # Use UTC and normalize format; avoid timezone offsets for SQLite comparisons
        cutoff_iso = (datetime.utcnow() - timedelta(days=days)).isoformat(timespec="seconds")

        with self._get_db_connection() as conn:
            cursor = conn.cursor()
            # Remove datetime() wrapper to avoid parse failures on offsets; ISO8601 sorts lexicographically
            cursor.execute("""
                           SELECT COALESCE(SUM(completion_tokens), 0) as total_completion,
            ...
            return {
                "completion_tokens": result["total_completion"],
                "prompt_tokens": result["total_prompt"],
    ...
    def get_usage_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        with self._get_db_connection() as conn:
            ...
            # Overall stats
            cursor.execute("""
                           SELECT SUM(completion_tokens)     as total_completion,
    ...
    for call_index, call in enumerate(lm_history):
        ...
        try:
            ...
            if isinstance(raw_timestamp, str):
                ...
                timestamp = raw_timestamp.isoformat() if hasattr(raw_timestamp,
                                                                 'isoformat') else datetime.now().isoformat()
        except Exception:
